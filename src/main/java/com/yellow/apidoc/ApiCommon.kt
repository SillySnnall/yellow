...
object ApiCommon {
    ...
    private fun getApiP(interfName: String): List<RequestParam> {
        val requestParamList: ArrayList<RequestParam> = arrayListOf()
        val methods = Class.forName("com.yellow.requestp.${Util.toUpperFristChar(interfName)}P").declaredMethods
        methods
                .map { it.annotations[0] as ApiRequestParamDoc }
                .forEach {
                    if (apiList.size != 0) {
